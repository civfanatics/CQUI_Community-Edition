...
function GetYieldForCity(yieldIndex:number, city:table, originCity:boolean)
    ...
    -- Adriaman
    -- CQUI Note: This is taken from fixes made by a user of BTS on the BTS Steam Workshop Page (Adriaman)
    ...
    local totalBeforeMultiplier:number = totalValue;
    local total:number = totalBeforeMultiplier;
    local multiplier:number = kYieldMultiplier;
    ...
    if total > 0 and multiplier ~= 1 then
        ...
        sourceText = sourceText .. Locale.Lookup("LOC_ROUTECHOOSER_YIELD_SOURCE_MULTIPLIERS", valueFromMultiplier, yieldInfo.IconString, yieldInfo.Name, multiplierAsPercent);
