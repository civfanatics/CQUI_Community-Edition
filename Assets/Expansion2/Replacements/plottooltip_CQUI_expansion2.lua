...
function GetDetails(data)
            ...
            if(playerResources:IsResourceVisible(resourceHash)) then
                ...
                if (resourceTechType ~= nil) then
                    ...
                    if (techType ~= nil and playerTechs:HasTech(techType.Index)) then
                        -- check if improved
                        if (data.DistrictType ~= nil and not data.DistrictPillaged) or (data.ImprovementType ~= nil and not data.ImprovementPillaged) then 
                            ...
                            if (kConsumption ~= nil and kConsumption.Accumulate) then
                                ...
                                if (iExtraction > 0) then
                                    ...
                                    table.insert(details, Locale.Lookup("LOC_RESOURCE_ACCUMULATION_BUILD_IMPROVEMENT", iExtraction, resourceIcon, "[COLOR:Civ6Red]"..LL(resource.Name).."[ENDCOLOR]")); -- {1_Amount} {2_Icon} {3_ResourceName} per turn
                                end
                            end
                        else
                            if data.Owner == localPlayerID then
                                table.insert(details, "[COLOR:Civ6Red]".."Improve or repair the tile to extract the resource".."[ENDCOLOR]"); -- TODO: move to LOC
                            end
                        end -- if improved
                    end -- has tech
                end
    ...
    local function ShowYields(details:table, yields:table)
        for k,v in pairs(yields) do print(k,v); end
